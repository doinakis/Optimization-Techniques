...
i = 1;
while (l < 0.1)
    [~,~,calc_number1(i)] = fibonacci_method(a_init,b_init,l,f1);
    [~,~,calc_number2(i)] = fibonacci_method(a_init,b_init,l,f2);
    [~,~,calc_number3(i)] = fibonacci_method(a_init,b_init,l,f3);
    l_values = [l_values l];
    l = l + l_step;
    i = i + 1;
end
...
for i=1:3
    ...
    %saveas(model,['./Diagrams/Subject3rd/f_',num2str(i),'_calculations_variable_l.jpeg']);
...
for i = 1:3
    ...
    [a1,b1,~] = fibonacci_method(a_init,b_init,l(i),f1);
    [a2,b2,~] = fibonacci_method(a_init,b_init,l(i),f2);
    [a3,b3,~] = fibonacci_method(a_init,b_init,l(i),f3);
    figure(figure_f1);
    hold on 
    grid on
...
%saveas(model,'./Diagrams/Subject3rd/f_1_intervals_variable_l.jpeg');
model = figure(figure_f2);
legend([h2(1),h2(2),h2(3)],'l=0.01','l=0.05','l=0.1');
%saveas(model,'./Diagrams/Subject3rd/f_2_intervals_variable_l.jpeg');
model = figure(figure_f3);
legend([h3(1),h3(2),h3(3)],'l=0.01','l=0.05','l=0.1');
%saveas(model,'./Diagrams/Subject3rd/f_3_intervals_variable_l.jpeg');