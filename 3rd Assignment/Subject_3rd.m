...
ylabel("f(xk,yk)");
% saveas(model,'./Diagrams/Subject_3rd/projection_function_values.jpeg');

% Show the difference with sk = 6, to show it converges
sk = 6;
[xk_projection,iterations_projection] = steepest_descent_with_projection(X,epsilon,f,gamma_value,a,b,sk,max_iterations);
model = figure(figure_counter);
figure_counter = figure_counter + 1;
...
xticks([0:5:iterations_projection-1 iterations_projection]);
ylabel("f(xk,yk)");
% saveas(model,'./Diagrams/Subject_3rd/projection2_function_values.jpeg');
